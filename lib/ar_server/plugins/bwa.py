...
class BwaAligner(BaseAligner, IPlugin):
    def run(self, contigs, reads, merged_pair=False):
        ...
        if len(reads) == 2:
            ...
            cmd_args = [self.executable, 'aln', contigs, reads[1],
                        '-f', aln_file2]
            self.arast_popen(cmd_args)

        # Create SAM
            cmd_args = [self.executable, 'sampe', contigs, 
                        aln_file1, aln_file2,
                        reads[0], reads[1],
                        '-f', samfile]
            self.arast_popen(cmd_args)
            ...
            cmd_args = [self.executable, 'samse', contigs, 
